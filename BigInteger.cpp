...
string BigInteger::add(string number1, string number2)
{
	...
	char carry = '0';
	int differenceInLength = abs( (int) (number1.size() - number2.size()) ); //计算需要在头部添加0的个数

	if(number1.size() > number2.size())//位数小的需要位数对齐（头部填0）
...
{
	string sub = (number1.length()>number2.length())? number1 : number2;//位数高的作为被减数
	int differenceInLength = abs( (int)(number1.size() - number2.size()) );//用于计算数据高位补0

	if(number1.size() > number2.size())	//执行补0
	...
	{
		if(number1[i] < number2[i])//如果当前位被减数不够，执行向上借位
		{
			number1[i] += 10;
			number1[i-1]--;
		}
	...
	while(sub[0]=='0' && sub.length()!=1) //消除头部多余的0
		sub.erase(0,1);
...
{
	if(n1.length() > n2.length())//保证n1为较小的数
		n1.swap(n2);

	string res = "0";
	for(int i=n1.length()-1; i>=0; --i)//n1是位数最小
	{
		string temp = n2;
		int currentDigit = n1[i]-'0';
		int carry = 0;
		//n1的每一位 与n2的每一位相乘，相乘结果需要在数据低位增加0，最后每一个值相加即可得出结果
		...
		temp.append((n1.length()-i-1), '0'); //数据低位增加0，增加当前计算的n1数的第几位
		res = add(res, temp); //相加
	}

	while(res[0] == '0' && res.length()!=1) //去除头部的0
		res.erase(0,1);
	return res;
}
